...
void initializeMatrix(int size , double matrix[][size]) ;
...
int main(int argc, char** argv) {
 ...
 if ( rank ==0 && argc < 2) {
     ...
     int chunkSize  = (matrixSize*matrixSize)/numworkers ;

    double matrix_a[matrixSize][matrixSize], matrix_b[matrixSize][matrixSize] ;
   ...
   // The below section initialize and generate matrix_a
    ...
    //     matrix_output[c] = (double *) malloc(matrixSize * sizeof ( double));
     ...
     double matrix_output[matrixSize][matrixSize];
     initializeMatrix(matrixSize,matrix_output);
     // printf("\nMatrix C \n");
    ...
    for (t = 1; t <= numworkers; t++) {
        ...
        int idx = 0 ;        
        for (idx = s; idx <= e ; idx++) {
              int rowNum = (idx) / ( matrixSize );
              int colNum = (idx) % ( matrixSize ) ;
              //int oldValue = matrix_output[rowNum][colNum] ;
              matrix_output[rowNum][colNum] = resultArray[idx-s] ;
              //int newValue = matrix_output[rowNum][colNum] ;
              //printf("%d (%d %d )->%d | (%d, %d)\n",idx, rowNum , colNum, resultArray[idx-s],oldValue,newValue);
        }
        printf("\n");
    }
  ...
  else  /**************************** worker task ************************************/
  {
      ...
      //printf("%d -  %d -  %d \n", rank, start_element_index, last_element_index);
       ...
       //  if (rowNum < matrixSize && colNum < matrixSize)
        ...
        resultArray[p-start_element_index]= res ;
       }
